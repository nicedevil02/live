...
- [ ]  **`B-01` — `snapshot()` را کش‌خوان محض کن.**
...
- [ ]  **`B-02` — واکشی را کامل به کرون بسپار.**
...
- [ ]  **`B-04` — تولید کد فعال‌سازی را اصلاح کن.**
...
- [ ]  **`B-05` — در لحظهٔ جفت شدن، دستگاه دائمی بساز.**
...
- [ ]  **`B-06` — اندپوینت ضربان قلب بساز.** این تسک سه کار را در یک درخواست جمع می‌کند: بررسی ابطال، بررسی آپدیت، و ریموت‌کانفیگ (`D-05`).
در `routes/api.php`:
...
- [ ]  **`B-07` — مدیریت دستگاه‌ها در پنل.** در پنل مدیریت (`app/Http/Controllers/Admin/`) یک بخش «تلویزیون‌های من» اضافه کن که فهرست `tv_devices` کاربر جاری را نشان بدهد با: برچسب قابل ویرایش، آخرین بازدید به تاریخ جلالی (از `User::toJalali` موجود استفاده کن)، نسخهٔ اپ، و دکمهٔ «حذف دستگاه» که `revoked_at` را پر می‌کند.
...
- [ ]  **`B-08` — `ensureTvSessionsTable()` را حذف کن.** فقط **بعد از** اینکه `R-04` و `B-03` تأیید کردند مایگریشن‌ها روی سرور سالم اجرا می‌شوند. این متد در هر درخواست یک `Schema::hasTable` می‌زند و یک کوئری اضافه به هر بار لود تابلو تحمیل می‌کند.
**معیار پذیرش:** بعد از حذف، جفت‌سازی کامل روی سرور پروداکشن یک بار دیگر تست شود و کار کند.
- [ ]  **`B-09` — فقط اگر `R-03` نشان داد کش روی `database` است.** درایور کش را به `file` تغییر بده (`CACHE_STORE=file`). روی cPanel اشتراکی `file` تقریباً همیشه از `database` سریع‌تر است و قفل‌های `Cache::lock` را از MySQL بیرون می‌برد. اگر هاست Redis دارد، Redis اولویت دارد.
...
- [ ]  **`W-01` — پارامتر `?tv=1` را به ویو برسان.**
...
- [ ]  **`W-03` — سرویس‌ورکر را در حالت تلویزیون خاموش کن.**
...
- [ ]  **`W-04` — خوانایی از فاصلهٔ سه متری.**
...
- [ ]  **`W-05` — پل خبررسانی JS به اپ نیتیو. این مهم‌ترین تسک فاز ۲ است.**
...
- [ ]  **`A-01` — اسکلت پروژه با صفر وابستگی.**
...
- [ ]  **`A-04` — `Config.kt` — آدرس پایه هرگز هاردکد نیست.**
...
- [ ]  **`A-06` — `PairingActivity` — صفحهٔ جفت‌سازی نیتیو.**
...
- [ ]  **`A-07` — `BoardActivity` و `TalaWebViewClient` — قلب پایداری.**
...
- [ ]  **`A-09` — پل `TalaTvBridge` و دیده‌بان تپش. دومین تسک حیاتی این فاز.**
...
- [ ]  **`A-10` — دروازهٔ نسخهٔ WebView و صفحهٔ تشخیص.**
...
یک `View` نیتیو روی WebView (نه صفحهٔ HTML، چون در قطعی شبکه ممکن است لود نشود):
    ...
    **معیار پذیرش:** کابل شبکه یا وای‌فای را قطع کن. لایهٔ آفلاین با ساعت آخرین نرخ ظاهر شود. شبکه را برگردان. تابلو **خودش** در کمتر از ۳۰ ثانیه برگردد، بدون لمس ریموت.
    > 
- [ ]  **`A-12` — نگهبان حافظه و ریلود شبانه.**
...
- [ ]  **`A-14` — تشخیص ساعت غلط و خطای گواهی.**
...
- [ ]  **`U-01` — کی‌استور: یک بار بساز، تا ابد نگه دار.**
...
- [ ]  **`U-02` — کانال آپدید درون‌برنامه‌ای.**
...
- [ ]  **`U-05` — مستندات و به‌روزرسانی فایل‌های مخزن.**
