...
<body class="bg-slate-50 dark:bg-[#020617] text-slate-800 dark:text-slate-100 flex flex-col justify-between min-h-screen py-6 sm:py-10 px-4 transition-colors duration-300">
    ...
    <div class="w-full max-w-md mx-auto my-auto space-y-5">
        ...
        {{-- سربرگ: لوگو، تغییر تم و بازگشت به صفحه اصلی --}}
        <div class="flex items-center justify-between gap-3 px-1">
            <a href="/" class="flex items-center gap-2.5 group shrink-0">
                <img src="{{ asset('images/logo.png') }}" class="h-9 w-9 object-contain rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1 shadow-xs" alt="طلالایو">
                <div>
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" id="themeToggleBtn" aria-label="تغییر تم"
                        class="w-9 h-9 rounded-xl flex items-center justify-center border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-600 dark:text-amber-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all shadow-xs cursor-pointer">
                ...
                <a href="/" class="text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center gap-1 px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs">
                    <span>صفحه اصلی</span>
                    <span>&larr;</span>
                </a>
            </div>
        ...
        {{-- کارت اصلی فرم ورود --}}
        <div class="bg-white dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800/80 rounded-3xl p-6 sm:p-7 shadow-xl space-y-5 transition-all">

            {{-- پیام‌های موفقیت یا خطا سمت سرور --}}
            @if (session('success'))
                <div class="bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-300 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 rounded-2xl p-3.5 text-xs flex items-center gap-2.5">
            ...
            @if ($errors->any())
                <div class="bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-800/80 text-rose-700 dark:text-rose-300 rounded-2xl p-3.5 text-xs flex items-center gap-2.5">
            ...
            <div id="otpSuccessAlert" class="hidden bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-300 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 rounded-2xl p-3 text-xs items-center gap-2">
                <span class="text-emerald-500 font-bold">✓</span>
                <span id="otpSuccessText"></span>
            </div>

            <div id="otpErrorAlert" class="hidden bg-rose-50 dark:bg-rose-950/50 border border-rose-300 dark:border-rose-800 text-rose-800 dark:text-rose-300 rounded-2xl p-3 text-xs items-center gap-2">
            ...
            {{-- ۱. فرم اصلی: ورود با رمز عبور --}}
            <div id="passwordSection" class="space-y-4 {{ $initialMode === 'password' ? '' : 'hidden' }}">
                <div class="pb-1">
                    <h1 class="text-base sm:text-lg font-black text-slate-900 dark:text-white">ورود به پنل مدیریت</h1>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">شماره موبایل یا نام کاربری و رمز عبور خود را وارد نمایید.</p>
                </div>
                ...
                <form method="POST" action="{{ route('admin.login') }}" class="space-y-4" id="passwordLoginForm">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative w-full" style="position: relative; width: 100%;">
                            <input type="password" name="password" id="loginPasswordInput" required minlength="4"
                                   style="padding-right: 48px; padding-left: 16px;"
                                   class="w-full h-14 bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-2xl text-base sm:text-lg font-normal text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/15 transition-all text-left font-mono shadow-xs" dir="ltr"
                                   placeholder="رمز عبور شما">
                            <button type="button" id="togglePasswordBtn" aria-label="نمایش یا پنهان‌سازی رمز"
                                    style="position: absolute; right: 12px; top: 50%; transform: translateY(-50%); z-index: 20; background: transparent; border: none; display: flex; align-items: center; justify-content: center; width: 36px; height: 36px; cursor: pointer;"
                    ...
                    <div class="flex items-center justify-between pt-0.5">
                        <button type="button" id="forgotPasswordLink"
                                class="text-xs font-semibold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 hover:underline cursor-pointer flex items-center gap-1.5 py-1">
                            <span>🔑</span>
                    ...
                    {{-- دکمه ورود --}}
                    <button type="submit"
                            class="w-full h-14 rounded-2xl bg-gradient-to-r from-amber-500 via-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black text-base shadow-lg shadow-amber-500/20 active:scale-[0.99] transition-all cursor-pointer flex items-center justify-center gap-2 mt-2">
            ...
            {{-- ۲. فرم فراموشی رمز عبور و تنظیم رمز جدید با تأیید پیامک --}}
            <div id="otpSection" class="space-y-4 {{ $initialMode === 'otp' ? '' : 'hidden' }}">
                ...
                <form method="POST" action="{{ route('admin.login.otp') }}" id="otpLoginForm" class="space-y-4">
                    ...
                    {{-- شماره موبایل و دکمه ارسال پیامک (هر دو با ارتفاع استاندارد h-14) --}}
                    <div class="space-y-1.5">
                        ...
                        <div class="flex flex-col sm:flex-row gap-2.5">
                            <input type="tel" name="phone" id="loginPhoneInput" required
                                   class="flex-1 h-14 bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 text-base sm:text-lg text-slate-900 dark:text-white font-mono text-center sm:text-left placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/15 transition-all font-normal shadow-xs"
                                   dir="ltr" placeholder="09187009064" maxlength="11" value="{{ old('phone') }}"
                                   autocomplete="tel" inputmode="numeric">
                            
                            <button type="button" id="sendLoginOtpBtn"
                                    class="h-14 px-5 rounded-2xl text-xs sm:text-sm font-bold transition-all shrink-0 cursor-pointer bg-amber-500 hover:bg-amber-400 text-slate-950 shadow-md shadow-amber-500/20 flex items-center justify-center gap-1.5 active:scale-[0.99]">
                    ...
                    <div id="otpInputContainer" class="space-y-3.5 {{ (old('otp') || old('phone')) ? '' : 'hidden' }} pt-1">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="otp" id="loginOtpInput" maxlength="5" inputmode="numeric"
                                   class="w-full h-14 bg-white dark:bg-slate-950 border-2 border-amber-500/80 rounded-2xl px-4 text-center text-2xl sm:text-3xl font-bold tracking-widest text-amber-600 dark:text-amber-400 font-mono placeholder-slate-300 focus:outline-none focus:ring-4 focus:ring-amber-500/20 shadow-xs"
                        ...
                        <button type="submit" id="loginOtpSubmitBtn"
                                class="w-full h-14 rounded-2xl bg-gradient-to-r from-amber-500 via-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black text-base shadow-xl shadow-amber-500/20 transition-all active:scale-[0.99] cursor-pointer mt-1 flex items-center justify-center gap-2">
            ...
            {{-- لینک ثبت‌نام --}}
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800/80 text-center text-xs sm:text-sm text-slate-500 dark:text-slate-400">
        ...
        {{-- بخش پشتیبانی فنی طلالایو (فوتر تمیز، بدون شلوغی) --}}
        <div class="flex items-center justify-center gap-3 text-xs text-slate-500 dark:text-slate-400 flex-wrap px-2">
            <span>پشتیبانی:</span>
            <a href="tel:09187009064" class="inline-flex items-center gap-1 text-slate-700 dark:text-slate-200 hover:text-amber-600 dark:hover:text-amber-400 transition-colors font-semibold">
                <span>📞</span>
                <span dir="ltr" class="font-mono">0918 700 9064</span>
            </a>
            <span class="text-slate-300 dark:text-slate-700">&bull;</span>
            <a href="https://rubika.ir/talalive" target="_blank" class="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:underline font-semibold">
                <span>کانال روبیکا</span>
            </a>
        </div>

        {{-- کپی‌رایت --}}
        <div class="text-center text-[11px] text-slate-400 pb-2">
